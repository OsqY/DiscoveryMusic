...
public class ApiResultDTO<T>
{
    ...
    public static async Task<ApiResultDTO<T>> CreateAsync(
        IQueryable<T> source,
        PaginationDTO pagination
    )
    {
        if (
            !string.IsNullOrEmpty(pagination.FilterColumn)
            && !string.IsNullOrEmpty(pagination.FilterQuery)
            && IsValidProperty(pagination.FilterColumn)
        )
            source.Where(
                string.Format("{0}.startsWith(@0)", pagination.FilterColumn),
        ...
        if (!string.IsNullOrEmpty(pagination.SortOrder) && IsValidProperty(pagination.SortColumn))
        {
            pagination.SortOrder =
                !string.IsNullOrEmpty(pagination.SortOrder)
                && pagination.SortOrder.ToUpper() == "ASC"
                    ? "ASC"
                    : "DESC";
            source = source.OrderBy(
                string.Format("{0} {1}", pagination.SortColumn, pagination.SortOrder)
            );
        }
